...
- [x] b) Alle Elemente eines Arrays zu filtern
...
```hab es const gemacht und results genannnt, dann "zahlen", weil der array "zahlen" heißt, dann .filter, weil ich filtern muss und (x => 10) weil wir wollen, dass alle zahlen über 10 gefiltert werden, dann .map(x => x*2) damit sich die gefilterete Zahlen sich verdoppeln.
...
```function toggleHintergrund(), weil die function so heißt.
const background= document.getElementById("box") -> weil man box ID braucht -> if(box.style.backgroundColor === "white") wenn es white ist, dann mache  -> box.style.backgroundColor= "yellow"; dann wird es yellow --> else bleibt es white. 
...
```js
// Dein Code hier:
const avgNoten = noten.reduce((acc, note) => {return acc += note/note.length},0);
